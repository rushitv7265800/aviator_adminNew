...
const StickerTable = (props) => {
  ...
  const handleChange = () => {
    ...
    if (data.length > 0) {
      let arr = [...checkedBoxes]; // Create a new array to avoid mutating the state directly
      if (selectAll) {
        // Instead of using .slice, you can loop through all data items
        data.forEach((dataItem) => {
          arr.push(dataItem?._id);
        });
        setOpenDeleteCount(true);
      } else {
        arr = []; // Clear the selection if selectAll is false
        setOpenDeleteCount(false); // Close the delete count if selectAll is false
      }
      setCheckedBoxes(arr);
    }
  };
